...
% mu = f(x,u)
mu(1:7,1) = attitudeDynamicsEuler3quat(mu_prev(1:7,1),M,J,dt);
mu(8:10,1) = gyroBiasEuler(mu_prev(8:10,1),[0;0;0],dt);
A = jacobianest(@(x) [attitudeDynamicsEuler3quat(x(1:7),M,J,dt); gyroBiasEuler(x(8:10),zeros(3,1),dt)], mu_prev); % compute Jacobian (linearize dynamics about mu_prev)
cov = A*cov_prev*A' + Q;

% UPDATE
C = jacobianest(@(x) getSensors3quat(x,R(5:7,5:7),R(1:4,1:4)),mu); % compute Jacobian
K = cov*C'*inv(C*cov*C'+R); % Kalman Gain
Ey = getSensors3quat(mu,zeros(3,3),zeros(4,4)); % Expected Measurement, g(x,u) 
...
mu(1:4,1) = q;
% if mu(1,1)^2 + mu(2,1)^2 + mu(3,1)^2 > 1
%     mu(1:4,1) = mu(1:4,1)/norm(mu(1:4,1));
% else
%     mu(4,1) = sqrt( 1 - mu(1,1)^2 - mu(2,1)^2 - mu(3,1)^2);
% end
