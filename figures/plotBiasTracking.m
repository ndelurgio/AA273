function plotBiasTracking(t,x,mu_ekf4quat,mu_ekf3quat,mu_mekf,mu_ukf,mu_mukf)
...
plot(t(1:end-1),rad2deg(mu_mekf(8,:)))
plot(t(1:end-1),rad2deg(mu_ukf(8,:)))
plot(t(1:end-1),rad2deg(mu_mukf(8,:)))
ylabel("b_x [deg/s]")
legend(["True","EKF4","EKF3","MEKF","UKF","MUKF"])
...
plot(t(1:end-1),rad2deg(mu_mekf(9,:)))
plot(t(1:end-1),rad2deg(mu_ukf(9,:)))
plot(t(1:end-1),rad2deg(mu_mukf(9,:)))
ylabel("b_y [deg/s]")
% legend(["True","EKF 4 Quaternion","EKF 3 Quaternion","MEKF","UKF","MUKF"])
...
plot(t(1:end-1),rad2deg(mu_mekf(10,:)))
